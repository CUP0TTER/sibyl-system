...
OUTPUT = OUT / '2023级建筑学_建筑设计基础2_教学进度表_严格模板版.docx'
...
# 严格依据大纲16周教学内容展开；周一3学时、周三3学时。
THEORY = [
    '建筑抄绘理论与案例剖析：抄绘目的、方法、关键要点及经典建筑案例分析',
    '建筑抄绘实践（一）：建筑平面图识读，比例、尺度、轴线及空间组织分析',
    '建筑抄绘实践（一）：建筑立面、剖面及透视表达，形体与空间关系分析',
    '建筑抄绘实践（二）：空间序列、功能布局、流线组织及图线层级深化',
    '建筑抄绘实践（二）：成果完善、建筑分析、设计元素提炼及抄绘总结',
    '平面构成原理与练习（一）：点、线、面基本元素及视觉特征',
    '平面构成原理与练习（二）：对称、均衡、重复、渐变等形式法则',
    '平面构成原理与练习（三）：对比、调和、节奏、正负形及共生图形',
    '立体构成原理与练习（一）：块体组合、空间限定及体量关系',
    '立体构成原理与练习（二）：线材、面材构成及材料连接方式',
    '立体构成原理与练习（三）：综合形态、空间序列、光影及作品评析',
    '模型制作材料与工具讲解：材料特性、工具使用、安全操作及制作流程',
    '模型制作实践（一）：图纸转化、模型分解、底板定位及主体结构',
    '模型制作实践（一）：楼板、屋面、门窗、立面构件及空间关系',
    '模型制作实践（二）：细部、材质、色彩、场地环境及整体效果',
    '模型制作实践（二）与课程总结：成果展示、汇报评价及学习复盘',
]

PRACTICE = [
    '建筑抄绘准备：熟悉工具，选择案例，确定比例、图幅和版面',
    '完成经典建筑平面图抄绘，校核比例、尺度、轴线和空间关系',
    '完成建筑立面图、剖面图及外观透视图抄绘，统一图线表达',
    '深化平、立、剖图纸，修正构造细节、线条层级和图面问题',
    '完成建筑分析图、抄绘心得和成果版面，开展阶段展示与讲评',
    '点、线、面组合及基本骨格练习，形成多组平面构成草案',
    '运用对称、均衡、重复、渐变等法则完成主题构成练习',
    '完成正负形、共生图形或综合构成，优化版面并集中讲评',
    '完成块体切割、组合与空间限定练习，比较不同体量关系',
    '完成线材、面材立体构成，检查节点连接和结构稳定性',
    '完成综合立体构成作品，调整空间、比例、光影和展示方式',
    '开展材料切割、粘接与工具安全训练，确定模型比例和计划',
    '制作模型底板、定位线、主体结构、墙体及主要空间构件',
    '完成楼板、屋面、门窗和立面构件，校核比例与空间关系',
    '完善材质、色彩、场地及景观细部，修正模型工艺问题',
...
def build_document():
    ...
    r = info.add_run('课程名称 建筑设计基础2  班级 2023级建筑学  从2024年2月26日至2024年6月14日止')
    set_font(r, size=9.2)

    # 主表严格保持模板17列表头文字和左右分栏结构。
    ...
    # 模板原有18个周次行保留；本课程填写1—16周，17—18周留空。
    for week in range(1, 19):
        ...
        for i, value in enumerate(values):
            ...
            size = 6.9 if left_align else 7.2
    ...
    r = footer.add_run('上课时间/地点：周一下午5、6、7节，周三下午7、8、9节/1B-305')
    set_font(r, size=9.0)
